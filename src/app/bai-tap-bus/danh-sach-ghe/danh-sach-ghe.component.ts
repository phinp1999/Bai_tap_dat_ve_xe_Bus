...
export class DanhSachGheComponent implements OnInit {
  @ViewChildren (ItemGheComponent) tagList: QueryList<ItemGheComponent>;
  ...
  viTriGhe(SoGhe){
    return this.mangGheDaDat.findIndex(item => {
      return item.SoGhe === SoGhe;
    });
  }

  datGhe(trangThaiChon, ghe){
    if(trangThaiChon){
      this.soGheDaDat++;
      ...
      this.soGheDaDat--;
      this.mangGheDaDat.splice(this.viTriGhe(ghe.SoGhe), 1);
    }
  }
  ...
  huyGhe(SoGhe){
    ...
    this.soGheDaDat--;
    this.mangGheDaDat.splice(this.viTriGhe(SoGhe), 1);
  }

  thanhTien(){
    this.tongTien = 0; 
    this.mangGheDaDat.map(item =>{
